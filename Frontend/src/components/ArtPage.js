...
const ArtPage = () => {
  ...
  const [messages, setMessages] = useState(() => ([
    { text: 'Журнал өчен мәкалә темасын кертегез', isUser: false, timestamp: new Date().toISOString() }
  ]));
  ...
  const handleSend = async () => {
    ...
    try {
      const resp = await axios.post(`${API_URL}/creators/journey`, { prompt }, {
          ...
          <div className="messages-area">
            ...
            {pdfUrl && (
              <div style={{ width: '100%', height: '100%', minHeight: 480, marginTop: 10 }}>
                <iframe title="art-pdf" src={pdfUrl} style={{ width: '100%', height: '100%', border: 'none' }} />
              </div>
            )}
          </div>
        </div>
          ...
          <div className="input-field compact-input-field">
            ...
            <div className="input-bottom">
              <div className="left-controls">
                {/* пусто — без прикрепления и выбора ИИ */}
              </div>
